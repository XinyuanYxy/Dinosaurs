...
function Human(){
    ...
    Creature.call(this,name,weight,humanHeightinInches,diet,'/images/human.png');
...
Dino.prototype.weightCompare = function (human) {
    if (human.weight > this.weight) {
        return `${this.name} is slimmer than you, ${human.name}, May be you should consider picking up a sport`;
    } else if (human.weight < this.weight) {
        return `${this.name} is heavier than you, ${human.name}!`;
    } else {
        return `${this.name} has the weight as you, ${human.name}.`;
    }
};
